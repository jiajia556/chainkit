...
type List struct {
	*models.BaseList[*ChainTokenGroups, *Record]
}
...
func NewList(session ...mysqlx.Session) *List {
	...
	if mysqlx.AutoCreateTable() {
		...
		err := createTableSession.CreateTableIfNotExists(new(ChainTokenGroups))
		if err != nil {
			panic(err)
		}
	}
	records := make([]*ChainTokenGroups, 0)
	l := &List{
		BaseList: &models.BaseList[*ChainTokenGroups, *Record]{
			Session: dbSession,
			Records: &records,
