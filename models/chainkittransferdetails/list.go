...
func (l *List) FindByFromAddressIdAndStatus(fromAddressId uint64, status int8, count int) error {
	return l.DB().Where("from_address_id = ? AND status = ?", fromAddressId, status).
		Order("id ASC").
